 ...
 */
export async function handleSearchDevice510K(args) {
    try {
        ...
        throw new Error(`Device 510(k) search failed: ${error.message}`);
    }
}
/**
 * Handles searching FDA device classifications
 */
export async function handleSearchDeviceClassifications(args) {
    try {
        ...
        throw new Error(`Device classification search failed: ${error.message}`);
    }
}
/**
 * Handles searching FDA device adverse events
 */
export async function handleSearchDeviceAdverseEvents(args) {
    try {
        ...
        throw new Error(`Device adverse event search failed: ${error.message}`);
    }
}
/**
 * Handles searching FDA device recalls
 */
export async function handleSearchDeviceRecalls(args) {
    try {
        ...
        throw new Error(`Device recall search failed: ${error.message}`);
    }
}
// Query building functions for device endpoints
function buildDevice510KQuery(args) {
    const queryParts = [];
    if (args.device_name) {
    ...
    if (args.decision_date_from || args.decision_date_to) {
        const dateQuery = buildDateRangeQuery('decision_date', args.decision_date_from, args.decision_date_to);
        if (dateQuery)
            queryParts.push(dateQuery);
    }
    return queryParts.length > 0 ? queryParts.join(' AND ') : '';
}
function buildDeviceClassificationQuery(args) {
    const queryParts = [];
    if (args.device_name) {
        queryParts.push(`device_name:"${args.device_name}"`);
    }
...
function buildDeviceAdverseEventQuery(args) {
    const queryParts = [];
    if (args.device_name) {
    ...
    if (args.date_from || args.date_to) {
        const dateQuery = buildDateRangeQuery('date_received', args.date_from, args.date_to);
        if (dateQuery)
            queryParts.push(dateQuery);
    }
    return queryParts.length > 0 ? queryParts.join(' AND ') : '';
}
function buildDeviceRecallQuery(args) {
    const queryParts = [];
    ...
    if (args.date_from || args.date_to) {
        const dateQuery = buildDateRangeQuery('recall_initiation_date', args.date_from, args.date_to);
        if (dateQuery)
            queryParts.push(dateQuery);
    }
    return queryParts.length > 0 ? queryParts.join(' AND ') : '';
...
function formatDevice510K(item) {
    return {
...
function formatDeviceClassification(item) {
    return {
        product_code: item.product_code,
        device_name: item.device_name,
...
function formatDeviceAdverseEvent(item) {
    return {
        report_number: item.report_number,
        event_type: item.event_type,
        date_received: item.date_received,
        device: item.device?.map((d) => ({
...
function formatDeviceRecall(item) {
    return {
