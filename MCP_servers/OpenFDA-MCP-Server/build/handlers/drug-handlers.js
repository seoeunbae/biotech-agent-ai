 ...
 */
export async function handleSearchDrugAdverseEvents(args) {
    try {
 ...
 */
export async function handleSearchDrugLabels(args) {
    try {
 ...
 */
export async function handleSearchDrugNDC(args) {
    try {
 ...
 */
export async function handleSearchDrugRecalls(args) {
    try {
 ...
 */
export async function handleSearchDrugsFDA(args) {
    try {
 ...
 */
export async function handleSearchDrugShortages(args) {
    try {
...
function buildDrugAdverseEventQuery(args) {
    const queryParts = [];
    if (args.drug_name) {
...
function buildDrugLabelQuery(args) {
    const queryParts = [];
    if (args.brand_name) {
...
function buildDrugNDCQuery(args) {
    const queryParts = [];
    if (args.product_ndc) {
...
function buildDrugRecallQuery(args) {
    const queryParts = [];
...
function buildDrugsFDAQuery(args) {
    const queryParts = [];
...
function buildDrugShortageQuery(args) {
    const queryParts = [];
...
function formatDrugAdverseEvent(event) {
    return {
        ...
        drugs: event.patient?.drug?.slice(0, 3)?.map((drug) => ({
        ...
        reactions: event.patient?.reaction?.slice(0, 3)?.map((reaction) => ({
            term: reaction.reactionmeddrapt,
            outcome: reaction.reactionoutcome
        })) || []
    };
}
function formatDrugLabel(label) {
    return {
        id: label.id,
        set_id: label.set_id,
...
function formatDrugNDC(ndc) {
    return {
        ...
        packaging: ndc.packaging?.slice(0, 2)?.map((pkg) => ({
            package_ndc: pkg.package_ndc,
            description: pkg.description
        })) || []
    };
}
function formatDrugRecall(recall) {
    return {
...
function formatDrugsFDA(drug) {
    return {
        ...
        generic_name: drug.openfda?.generic_name?.[0],
        manufacturer: drug.openfda?.manufacturer_name?.[0],
        products: drug.products?.slice(0, 2)?.map((product) => ({
        ...
        submissions: drug.submissions?.slice(0, 2)?.map((sub) => ({
...
function formatDrugShortage(shortage) {
    return {
